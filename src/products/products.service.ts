...
export class ProductsService {
    ...
    async updateProduct(id: string, {
        ...
        if (productQuantity <= 0 || productPrice <= 0) {
            throw new HttpException('Given number is less or equal 0', HttpStatus.BAD_REQUEST);
        }

        if (productName.length <= 3) {
            throw new HttpException(`Product name should be at least 4 characters. Got ${productName.length}`, HttpStatus.BAD_REQUEST);
        }
