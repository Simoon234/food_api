...
export class TableReservationService {
        ...
        switch (chosenNumberOfTable) {
            case 2:
                if (reservationAdminDetails.doubleTable <= 0) {
                    throw new HttpException('Sorry, we do not have available tables', HttpStatus.BAD_REQUEST);
                }
    ...
    async reserveTable(id: string, {
        ...
        if (chosenNumberOfTable !== (2 || 4 || 6)) {
            throw new HttpException('We do not have that kind of table..Available tables are [2, 4, 6]', HttpStatus.NOT_FOUND);
        }
        ...
        const getAllReservations = await Reservation.find({relations: ['reservedBy']});


        getAllReservations.map((item) => {
            if (item.reservedBy.id === user.id) {
                throw new HttpException('You have reservation.', HttpStatus.BAD_REQUEST);
            }
        })
    ...
    async changedTableStatus(id: string): Promise<Res> {
        const getReservation = await Reservation.findOne({where: {id}, relations: ['reservedBy']});

        if (getReservation === null) {
            throw new HttpException('Can not find reservation', HttpStatus.BAD_REQUEST);
        }
        ...
        await Reservation.createQueryBuilder().delete().from(Reservation).where("id = :id", {id}).execute()
